...
export const LanguageSwitcherMobile: React.FC = () => {
    ...
    return (<>
        <div style={{ width: '100%' }} >
            <Accordion childs={ Object.entries(Languages).map(([key, value]) => <div style={{ paddingLeft: '20px', marginTop: '20px' }} onClick={ () => { SwitchLanguage(key as Languages) } }>{t(value)}</div>) } order={'Jazyk'}/>
        </div>
    </>)
}
